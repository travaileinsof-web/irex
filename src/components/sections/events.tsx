                  ...
                  <div className={`ml-12 md:ml-0 md:w-1/2 ${i % 2 === 0 ? "md:pr-12" : "md:pl-12"}`}>
                    ...
                    >
                      ...
                      <div className="relative h-32 overflow-hidden">
                        <img src={eventImages[i % eventImages.length]} alt={event.name} loading="lazy" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div className="absolute inset-0 bg-gradient-to-t from-obsidian/60 to-transparent" />
                        {/* Type badge */}
                        <div className="absolute left-3 top-3">
                          <span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider backdrop-blur-sm ${typeColors[event.type] ?? "bg-gold/20 text-gold border-gold/40"}`}>
                            {event.type}
                          </span>
                        </div>
                      </div>
