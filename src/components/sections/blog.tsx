            ...
            <Reveal key={i} delay={(i % 4) * 0.08}>
              ...
              >
                ...
                <div className="relative aspect-[16/10] overflow-hidden">
                  <img src={blogImages[i % blogImages.length]} alt={post.title} loading="lazy" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                  <div className="absolute inset-0 bg-gradient-to-t from-coal via-coal/30 to-transparent" />
                  {/* Category badge */}
                  <div className="absolute left-3 top-3 rounded-full bg-gold/90 backdrop-blur-sm px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-obsidian">
                    {post.category}
                  </div>
                </div>
