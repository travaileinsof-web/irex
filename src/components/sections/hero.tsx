...
/** Deterministic seeded random — same values on server & client to avoid hydration mismatch */
function seededRandom(seed: number) {
  const x = Math.sin(seed * 9999) * 10000;
  return x - Math.floor(x);
}

// SSR-safe "mounted" detection via useSyncExternalStore (no setState in effect)
...
/**
 * Hero — full-screen premium intro with image background, parallax, particle field, animated headline
 */
export function Hero() {
  ...
  // Particles render only after client mount (useSyncExternalStore avoids setState-in-effect lint error)
  const mounted = useSyncExternalStore(emptySubscribe, getMountedSnapshot, getServerSnapshot);

  // Deterministic particle field — generated once via useMemo
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(212, 165, 71, 0.18), transparent 70%)",
          scale,
        }}
      />
      ...
      {/* Particle field — only rendered client-side after mount to avoid SSR hydration mismatch */}
